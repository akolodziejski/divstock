...
def test_find_header_row_skips_metadata(tmp_path):
    ...
    f.write_text(
        ...
        "Account,Data,U12345\n"
        "DividendDetail,DataDiscriminator,Currency,Symbol,ReportDate,Gross,Withhold\n"
        "DividendDetail,Data,Summary,CAD,BNS,20210428,9.0\n",
        encoding="ISO-8859-1",
    )
...
def test_load_csvs_ib_format(tmp_path):
    ...
    csv.write_text(
        ...
        "DividendDetail,DataDiscriminator,Currency,Symbol,ReportDate,Gross,Withhold\n"
        "DividendDetail,Data,Summary,CAD,BNS,20210428,9.0\n"
        "DividendDetail,Data,Summary,USD,AAPL,20210510,12.5\n",
        encoding="ISO-8859-1",
    )
    dfs = load_csvs(tmp_path)
